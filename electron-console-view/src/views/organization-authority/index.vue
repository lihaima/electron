<template>
	<div class="app-container">
		<div>
			...
			<!-- 表格详情 -->
			<el-button type="primary" size="small" icon="el-icon-plus" @click="() => { $refs.validateUserSimplePopup.handleCreate() }" class="mt-20">新增</el-button>
		...
		<!-- 验证用户信息 -->
		<simple-popup ref="validateUserSimplePopup" :settings="validateUserPopupInfo" v-on="validateUserPopupInfo.events"></simple-popup>
	</div>
</template>
				...
				tableInfo: {
					...
					events: {
						...
						delete: row => {
							...
							this.$confirm('确定删除该权限？', '操作确认', {confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning'}).then(() => {
								// Todo: 事件操作
						...
						items: [
							{
								...
								rules: [{ required: true, message: '请输入权限id', trigger: 'blur' }]
							},
							{
								props: 'oiName',
								type: 'input',
							...
							},
							{
								...
								rules: [{ required: true, message: '请输入权限类型', trigger: 'blur' }]
							},
							{
								props: 'oiExplain',
								type: 'input',
								...
								rules: [{ required: true, message: '请输入权限说明', trigger: 'blur' }]
							}
						]
					},
				...
				validateUserPopupInfo: {
					...
					events: {
						confirm: data => {
							...
							this.$refs.validateUserSimplePopup.handleClose()
